...
local function r_inspect(suffix, r_expr, desc)
    vsc({"n", "v"}, "<localleader>" .. suffix, "r.runCommandWithSelectionOrWord", "R: " .. desc, {r_expr})
end

--- Run an arbitrary R command in the active R session
---@param r_cmd string R code to execute
local function r_run(r_cmd)
    vscode.action("r.runCommand", {
        args = {r_cmd}
    })
end
...
r_inspect("s", "str($$)", "str")
r_inspect("g", "options(width = as.integer(system2('tput', 'cols', stdout = TRUE)));" ..
    "tryCatch(dplyr::glimpse($$), error = function(e) str($$))", "glimpse (with fallback)")
...
-------------------------------------------------------------------------------
-- Profiling (<localleader>p) via profvis
-------------------------------------------------------------------------------

r_inspect("ps",
    "if (!requireNamespace('profvis', quietly = TRUE)) stop('Package profvis is required. Install with install.packages(\"profvis\")');" ..
        "profvis::profvis({ $$ }, rerun = TRUE)", "Profile selection or word")

keymap("n", "<localleader>pl", function()
    local code = vim.api.nvim_get_current_line()
    local code_lit = string.format("%q", code)
    local r_cmd =
        "if (!requireNamespace('profvis', quietly = TRUE)) stop('Package profvis is required. Install with install.packages(\"profvis\")');" ..
            ".__pv_code <- " .. code_lit .. ";" ..
            "profvis::profvis(eval(parse(text = .__pv_code), envir = .GlobalEnv), rerun = TRUE)"
    r_run(r_cmd)
end, {
    silent = true,
    noremap = true,
    desc = "R: Profile current line"
})

keymap("n", "<localleader>pf", function()
    local file_path = vim.fn.expand("%:p")
    local file_lit = string.format("%q", file_path)
    local r_cmd =
        "if (!requireNamespace('profvis', quietly = TRUE)) stop('Package profvis is required. Install with install.packages(\"profvis\")');" ..
            "profvis::profvis(source(" .. file_lit .. ", local = .GlobalEnv), rerun = TRUE)"
    r_run(r_cmd)
end, {
    silent = true,
    noremap = true,
    desc = "R: Profile current file"
})
...
vsc({"n", "v"}, "<localleader>P", "r.runCommand", "R: print .Last.value", {".Last.value"})
...
vsc("n", "<localleader>br", "r.runCommand", "R: Restart R session", {"rstudioapi::restartSession()"})
vsc("n", "<localleader>bp", "workbench.action.tasks.runTask", "R: Pkgdown build site", {"R: Pkgdown"})
vsc("n", "<localleader>bv", "workbench.action.tasks.runTask", "R: Build Vignettes", {"R: Build Vignettes"})
vsc("n", "<localleader>bt", "workbench.action.tasks.runTask", "R: Tar Make", {"R: Tar Make"})
